...
chrome.spawn = function(options) {
	return new Promise((resolve, reject) => {
		...
		let flags = ['--headless', '--disable-gpu', '--remote-debugging-port=9222', '--hide-scrollbars'];
		if (this.options.chromeFlags) {
			flags = this.options.chromeFlags;
		}
		if ('PRERENDER_CHROME_FLAGS' in process.env) {
			flags.push(...(process.env.PRERENDER_CHROME_FLAGS?.split(' ') || []))
		}

		this.chromeChild = spawn(location, flags);
	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Network.requestWillBeSent((params) => {
				...
				if (params.type === "Document" && tab.prerender.initialFrameId === params.frameId && getOrigin(tab.prerender.url) !== getOrigin(params.request.url) && !tab.prerender.followRedirects && !this.options.followRedirects) {
